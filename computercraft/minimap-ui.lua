...
local TileGrid = dofile("minimap/tilegrid.lua").init({
  state = state,
  monitor = monitor,
  width = function() return width end,
  mapCenter = mapCenter,
  decodeTextRow = decodeTextRow,
  subW = SUB_W,
  subH = SUB_H,
})
...
local function overlayCell(col, row, stenBits, color, mapH, override)
  ...
  local packed_byte, cell_fg, cell_bg = TileGrid.getCell(col, row, mapH, mcx, mcz)
...
blitLabelOverMap = function(text, col, row, mapH, preferredFg, grid)
  ...
  for i = 1, writeLen do
    local _, _, bg = TileGrid.getCell(startCol + i - 1, row, mapH, mcx, mcz, grid)
...
local function drawTapeLabel(text, row, anchorCol, mapH)
  ...
  for i = 1, #text do
    ...
    if c >= 1 and c <= width then
      local _, _, bg = TileGrid.getCell(c, row, mapH, mcx, mcz)
      local bgChar = bg or "f"
...
local function fullRedraw()
  ...
  if state.screen == "map" then
    ...
    elseif state.zoomLoadingCenter and state.loadingFallback then
      TileGrid.drawCachedMap(mapH, state.loadingFallback)
